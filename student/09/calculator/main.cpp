...
int main() {
    ...
    while ( true ) {
        ...
        // Converting command name to uppercase:
        for(auto tt : command_to_be_executed)
        {
            char test = toupper(tt);

            command_to_be_executed.at(idx) = test;
            idx++;
        }
        bool to_exit = false;
        ...
        for(auto cmd : COMMANDS)
        {
            if(cmd.str == command_to_be_executed)
            {
                ...
                if(pieces.size() == cmd.parameter_number + 1)
                {
                    ...
                    {
                        if(string_to_double(pieces.at(1), operand_left) && string_to_double(pieces.at(2), operand_right))
                        {
                            cout << cmd.action(operand_left, operand_right) << endl;
                        }
                        else
                        {
